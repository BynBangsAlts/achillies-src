...
public class SilentAimModule extends Module {
    ...
    private void onHudRender(HudRenderEvent event) {
        if (fovCircle.isEnabled()) {
            RenderUtils.renderCircle(event.context.getMatrices(), new Color(255, 255, 255, 80), mc.getWindow().getScaledWidth() / 2, mc.getWindow().getScaledHeight() / 2, fov.getFValue() * 2, 60);
        }
    }
}
